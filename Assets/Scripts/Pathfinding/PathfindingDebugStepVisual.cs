...
public class PathfindingDebugStepVisual : MonoBehaviour {
    ...
    public void Setup(Grid<GridMapObject> grid) {
    ...
    public void TakeSnapshot(Grid<GridMapObject> grid, GridMapObject current, List<GridMapObject> openList, List<GridMapObject> closedList) {
        ...
        for (int x = 0; x < grid.GetWidth(); x++) {
            for (int y = 0; y < grid.GetHeight(); y++) {
                ...
                int gCost = gridMapObject.gCost;
                int hCost = gridMapObject.hCost;
                int fCost = gridMapObject.fCost;
                Vector3 gridPosition = new Vector3(gridMapObject.x, gridMapObject.y) * grid.GetCellSize() + Vector3.one * grid.GetCellSize() * .5f;
                bool isCurrent = gridMapObject == current;
                bool isInOpenList = openList.Contains(gridMapObject);
                bool isInClosedList = closedList.Contains(gridMapObject);
                int tmpX = x;
                int tmpY = y;
    ...
    public void TakeSnapshotFinalPath(Grid<GridMapObject> grid, List<GridMapObject> path) {
        GridSnapshotAction gridSnapshotAction = new GridSnapshotAction();
        gridSnapshotAction.AddAction(HideNodeVisuals);
        ...
        for (int x = 0; x < grid.GetWidth(); x++) {
            for (int y = 0; y < grid.GetHeight(); y++) {
                ...
                int fCost = gridMapObject.fCost;
                Vector3 gridPosition = new Vector3(gridMapObject.x, gridMapObject.y) * grid.GetCellSize() + Vector3.one * grid.GetCellSize() * .5f;
                bool isInPath = path.Contains(gridMapObject);
                int tmpX = x;
                int tmpY = y;
